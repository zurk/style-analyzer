...
class FormatModelTests(unittest.TestCase):
    def setUp(self):
        ...
        trainer = TrainableRules(**self.config)
    ...
    def test_save_and_load(self):
        ...
        with tempfile.NamedTemporaryFile(prefix="lookout-") as f:
            ...
            for lang in fm1.languages:
                ...
                self.assertEqual(fm1[lang]._config.keys(), fm2[lang]._config.keys())
                c = {k: v for k, v in fm1[lang].config["trainable_rules"].items() if v is not None}
                self.assertEqual(c, fm2[lang].config["trainable_rules"])
