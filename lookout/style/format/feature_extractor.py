...
from lookout.style.format.annotations.annotations import AccumulatedIntentationAnnotation, \
    PathAnnotation, TargetAnnotation, TokenAnnotation, UASTAnnotation
...
class FeatureExtractor:
    ...
    def _parse_file(self, file: AnnotatedData) -> \
            Tuple[List[VirtualNode], Dict[int, bblfsh.Node]]:
        """
        Parse a file into a sequence of `VirtuaNode`-s and a mapping from VirtualNode to parent.
        ...
        :param file: Source file text annotated with path and uast.
        :return: list of `VirtualNode`-s and the parents.
        """
        ...
        node_tokens = []
        parents = {}
        queue = [file.iget(UASTAnnotation, 0).uast]
        while queue:
            node = queue.pop()
            if node.internal_type in self.node_fixtures:
                node = self.node_fixtures[node.internal_type](node)
        ...
        for node in node_tokens:
            ...
            if node.start_position.offset > pos:
                ...
                for match in parser.finditer(diff):
                    ...
                    token = match.group()
                    sumlen += len(token)
                    file.add(TokenAnnotation(*offsets))
                assert sumlen == node.start_position.offset - pos, \
                    "missed some imaginary tokens: \"%s\"" % diff
            if node is sentinel:
                break
            uast_node_annot = list(VirtualNode.from_node(node, contents, self.token_unwrappers))
            annotated = AnnotatedData(contents)
            annotated.update(uast_node_annot)
            annotated.add(file.iget(PathAnnotation, 0))
            t1 = to_vnodes(annotated)
            t2 = list(VirtualNode.from_node_old(node, contents, file.iget(PathAnnotation, 0).path,
                                                self.token_unwrappers))
            for vn1, vn_old in zip(t1, t2):
                if vn1 != vn_old:
                    print("new: ", repr(vn1))
                    print("old: ", repr(vn_old))
                    print("!!!")
            file.update(uast_node_annot)
            pos = node.end_position.offset

        # backward
        result = to_vnodes(file)

        result_old, _ = self._parse_file_old(contents, file.iget(UASTAnnotation, 0).uast,
                                             file.iget(PathAnnotation, 0).path)
...
def to_vnodes(file: AnnotatedData) -> List["VirtualNode"]:
    """
    ...
    vnodes = []
    vnode_annotations = [TokenAnnotation, PathAnnotation, AccumulatedIntentationAnnotation,
                         TargetAnnotation]
    path = file.iget(PathAnnotation, 0).path
    ...
    for value, annotations in file.iter_items(vnode_annotations):
        vnode = VirtualNode(
            ...
            is_accumulated_indentation=AccumulatedIntentationAnnotation in annotations,
            path=path,
            node=annotations[TokenAnnotation].node,
            y=annotations[TargetAnnotation].target if TargetAnnotation is annotations else None,
        )
        vnodes.append(vnode)
    return vnodes
