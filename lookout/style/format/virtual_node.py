...
from lookout.style.format.annotations.annotations import Annotation, TokenAnnotation, \
    UASTNodeAnnotation
    ...
    def from_node(node: bblfsh.Node, file: str,
        ...
        if not node.token:
            if outer_token == "''" or outer_token == '""':
                ...
                yield TokenAnnotation(node.start_position.offset, middle)
                ...
                yield TokenAnnotation(middle, middle, uast_annotation)
                yield uast_annotation
                yield TokenAnnotation(middle, node.end_position.offset)
            else:
                uast_annotation = UASTNodeAnnotation.from_node(node)
                yield TokenAnnotation(
        ...
        if start_offset:
            yield TokenAnnotation(node.start_position.offset, start_pos)
        end_pos = start_pos + len(node_token)
        uast_annotation = UASTNodeAnnotation(start_pos, end_pos, node)
        yield TokenAnnotation(start_pos, end_pos, uast_annotation)
        yield uast_annotation
        if end_pos < node.end_position.offset:
            yield TokenAnnotation(end_pos, node.end_position.offset)
