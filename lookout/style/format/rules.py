...
class Rules:
    ...
    def __init__(self, rules: List[Rule], config: Mapping[str, Any]):
        """
        Initializes the rules so that it is possible to call predict() afterwards.

        :param rules: the list of rules to assign.
        :param config: all parameters that are used for the model training.
        """
        super().__init__()
...
class TrainableRules(BaseEstimator, ClassifierMixin):
    ...
    def __init__(self, *, base_model_name: str = "sklearn.tree.DecisionTreeClassifier",
        ...
        self._config = self._apply_config(config)
    ...
    def fit(self, X: numpy.ndarray, y: numpy.ndarray) -> "TrainableRules":
        ...
        self._rules = Rules(rules, self._config)
        return self
    ...
    def _apply_config(self, config: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        if config is None:
            params = {}
            config = {}
        else:
            params = dict(config["trainable_rules"])
        self.set_params(**params)
        config["trainable_rules"] = self.get_params(deep=False)
        config["trainable_rules"]['top_down_greedy_budget'] = \
            list(config["trainable_rules"]['top_down_greedy_budget'])
        return config
